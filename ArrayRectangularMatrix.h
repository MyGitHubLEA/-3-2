...
public:
    ...
    int GetNumberColumnsTest() const;
    T Get(int indexRow, int indexColumn) const;
    const T& GetTest(int indexRow, int indexColumn) const;

    void Set(T val, int indexRow, int indexColumn);
    void SetTest(T val, int indexRow, int indexColumn);
...
template<typename T>
int ArrayRectangularMatrix<T>::GetNumberColumnsTest() const
{
    return (this->items).GetTest(0).GetLength();
}
...
template<typename T>
const T& ArrayRectangularMatrix<T>::GetTest(int indexRow, int indexColumn) const
{
    if ((this->items).GetLength() == 0)
    {
        throw std::domain_error("Empty matrix");
    ...
    if (indexColumn >= this->GetNumberColumnsTest() || indexColumn < 0)
    {
        throw std::out_of_range("Out of the range of the array");
    }
    return ((this->items).GetTest(indexRow)).GetTest(indexColumn);
}
...
template<typename T>
void ArrayRectangularMatrix<T>::SetTest(T val, int indexRow, int indexColumn)
{
    if ((this->items).GetLength() == 0)
    {
        throw std::domain_error("Empty matrix");
    ...
    MutableArraySequence<T> newArray(this->items.GetTest(indexRow));
    newArray.Set(val, indexColumn);
    (this->items).Set(newArray, indexRow);
}
